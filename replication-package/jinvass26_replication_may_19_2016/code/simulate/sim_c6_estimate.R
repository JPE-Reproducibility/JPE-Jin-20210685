...
sev_csv_path <- file.path(MODEL_OUT_DIR, "model_sev", "results", "bootstrap-result-id-0.csv")
cache_warm_start <- USE_CACHE && file.exists(sev_csv_path)
...
sev_fit <- tryCatch({
  sev_model$optimize(
    ...
    iter = iter_use,
    threads = NUM_THREADS,
    init_alpha = if (cache_warm_start) 1e-8 else sev_init$init_alpha,
    tol_obj = TOL_OBJ,
...
price_csv_path <- file.path(MODEL_OUT_DIR, "model_price", "results", "bootstrap-result-id-0.csv")
cache_warm_start <- USE_CACHE && file.exists(price_csv_path)
if (cache_warm_start) cat("  Warm-start from cached output (reduced iterations)\n")
iter_use <- if (cache_warm_start) 100 else MAX_ITER
...
price_fit <- tryCatch({
  price_model$optimize(
    ...
    iter = iter_use,
    threads = NUM_THREADS,
    init_alpha = if (cache_warm_start) 1e-8 else price_init$init_alpha,
    tol_obj = TOL_OBJ,
    tol_rel_grad = if (cache_warm_start) 1e9 else 1e7,
    refresh = REFRESH
  )
...
## ============================================================================
## 2b. Estimate model_cost (moral hazard coefficients for model_main)
## ============================================================================

cat("\n================================================================\n")
cat("2b. Estimating model_cost\n")
cat("================================================================\n\n")

cost_csv_path <- file.path(MODEL_OUT_DIR, "model_cost", "results", "bootstrap-result-id-0.csv")
cache_warm_start <- USE_CACHE && file.exists(cost_csv_path)
if (cache_warm_start) cat("  Warm-start from cached output (reduced iterations)\n")
iter_use <- if (cache_warm_start) 100 else MAX_ITER

cost_stan_path <- file.path(STAN_MODEL_BASE, "model_cost", "model_cost.stan")
if (!file.exists(cost_stan_path)) {
  cat("  SKIP:", cost_stan_path, "not found\n")
} else {
  cat("Compiling model_cost...\n")
  cost_model <- cmdstanr::cmdstan_model(
    cost_stan_path,
    stanc_options = list("O1"),
    force_recompile = FALSE,
    cpp_options = list(stan_threads = TRUE)
  ...
  # model_cost uses the same data fields as model_cost_mhhet minus X_rc_clm
  # and with run_demand = 0 (cost-only, no demand likelihood)
  cost_data <- list()
  cost_fields <- c(
    ...
    "discount_factor", "score_sd_lb", "score_sd_ub",
  ...
  for (f in cost_fields) {
    if (f %in% names(sim_data_list) && !is.null(sim_data_list[[f]]))
      cost_data[[f]] <- sim_data_list[[f]]
  }
  if (is.null(cost_data$M_clm_record)) cost_data$M_clm_record <- sim_data_list$M
  if (is.null(cost_data$zip_income)) cost_data$zip_income <- rep(0, sim_data_list$N_choice)
  cost_data$run_demand <- 0L
  cost_data$run_demand_blocks <- rep(0L, sim_data_list$N_choice_regimes)
  cost_data$reg_factor_score <- rep(0.01, sim_data_list$D_R_tm_scheme)
  # eps_sd, eps_ra_sd, eps_eta_sd from real-data main model
  main_csv_for_cost <- file.path("data/estimates/model_output/model_main", "results", "bootstrap-result-id-0.csv")
  if (file.exists(main_csv_for_cost)) {
    .mp <- parse_bootstrap_csv(main_csv_for_cost)$params
    cost_data$eps_sd <- .mp$eps_sd
    cost_data$eps_ra_sd <- if (!is.null(.mp$eps_ra_sd)) .mp$eps_ra_sd else 0.1
    cost_data$eps_eta_sd <- if (!is.null(.mp$eps_eta_sd)) .mp$eps_eta_sd else 0.1
    rm(.mp)
  } else {
    cost_data$eps_sd <- 1.0; cost_data$eps_ra_sd <- 0.1; cost_data$eps_eta_sd <- 0.1
  }

  cat("Running L-BFGS optimization for model_cost...\n")
  cat("Start:", format(Sys.time()), "\n")
  cost_init <- build_init("model_cost")
  cost_fit <- tryCatch({
    cost_model$optimize(
      data = cost_data,
      init = cost_init$init,
      ...
      iter = iter_use,
      threads = NUM_THREADS,
      init_alpha = if (cache_warm_start) 1e-8 else cost_init$init_alpha,
      tol_obj = TOL_OBJ,
      tol_rel_grad = if (cache_warm_start) 1e9 else 1e7,
      refresh = REFRESH
    )
  ...
  if (!is.null(cost_fit)) {
    save_model_output(cost_fit, "model_cost", cost_stan_path)
    cat("  model_cost estimated successfully\n")
  } else {
    cat("  model_cost estimation FAILED\n")
  }
}
...
main_csv_path <- file.path(MODEL_OUT_DIR, "model_main", "results", "bootstrap-result-id-0.csv")
cache_warm_start <- USE_CACHE && file.exists(main_csv_path)
if (cache_warm_start) cat("  Warm-start from cached output (reduced iterations)\n")
iter_use <- if (cache_warm_start) 100 else MAX_ITER
...
main_fit <- tryCatch({
  main_model$optimize(
    ...
    iter = iter_use,
    threads = NUM_THREADS,
    init_alpha = if (cache_warm_start) 1e-8 else main_init$init_alpha,
    tol_obj = TOL_OBJ,
    tol_rel_grad = if (cache_warm_start) 1e9 else 1e7,
    refresh = REFRESH
  )
