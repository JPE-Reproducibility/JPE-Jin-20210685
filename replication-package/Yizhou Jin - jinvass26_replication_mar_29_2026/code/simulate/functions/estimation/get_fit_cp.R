...
## ---- Figure B3: Score vs Lambda (fig_b3.csv) --------------------------------

rr_tm = data_list$R_tm_scheme[I_tm_R_to_I]

df_fit_score_tm = rr_tm %>% as_tibble() %>% rename(rr = value) %>%
  mutate(llambda_severe_bin = llambda_severe_bin[I_tm_R_to_I],
         llambda_bin = llambda_bin[I_tm_R_to_I],
         leps_bin = leps_bin[I_tm_R_to_I],
         rc_bin = rc_bin[I_tm_R_to_I],
         rr = factor(rr),
         data = log_tm_score_R_ordered, model = log_score_est) %>%
  group_by(llambda_bin) %>% filter(n() > 10) %>% ungroup() %>%
  gather(type, vals, -c("llambda_severe_bin", "llambda_bin", "leps_bin", "rc_bin", "rr"))

fig_b3_csv <- df_fit_score_tm %>% group_by(llambda_bin, rr, type) %>%
  filter(n() > 25) %>%
  summarise(mean = mean(vals), se = sd(vals)/sqrt(n()), .groups = "drop")
