...
if (file.exists(fig_2a_csv)) {
  ...
  ggsave(gg, file = file.path(IMAGES_DIR, "fig_2a.png"), width = 1500/72, height = 900/72, dpi = 72)
  cat("  [GEN] fig_2a.png\n"); n_generated <- n_generated + 1
} else {
...
if (file.exists(fig_2b_csv)) {
  ...
  ggsave(gg, file = file.path(IMAGES_DIR, "fig_2b.png"), width = 1500/72, height = 900/72, dpi = 72)
  cat("  [GEN] fig_2b.png\n"); n_generated <- n_generated + 1
} else {
...
if (file.exists(fig_3_csv)) {
  ...
  ggsave(gg, file = file.path(IMAGES_DIR, "fig_3.png"), width = 1341/300, height = 896/300, dpi = 300)
  cat("  [GEN] fig_3.png\n"); n_generated <- n_generated + 1
} else {
...
if (file.exists(tab_1a_csv) && file.exists(tab_1b_csv) && file.exists(tab_1_meta_csv)) {
  ...
  hdr <- paste0(
    ...
    "\\multicolumn{8}{c}{(b) By Coverage and Firm (Illinois)}\\\\\n",
    "\\toprule\n",
  ...
  combined_tex <- paste0(combined_tex,
    ...
    "\\item (b) \\textit{Note: }This table reports the average quotes and claims of the monitoring firm and its top 5 competitors by market share in Illinois, which is the focus of our structural and counterfactual analyses. \n",
    "The total number of consumers is ", bigI, ". The total number of quotes is ", bigN_b, ".\n",
    "In Illinois, the mandatory minimum and the most popular coverage changed from \\$40,000 to \\$50,000 during the research window, which is why the former had lower share and slightly lower claims. \n",
...
if (file.exists(tab_a1a2_json)) {
  ...
  il_tex <- build_x_sumstat_table(j$il,
    "Summary Statistics on Observable Characteristics (Illinois)", "tab:sum_stat_x_il",
