...
if (!USE_CACHE) {
  # Clean generated output (not data cache unless FROM_SCRATCH)
  clean_contents(MODEL_OUT_DIR, pattern = "\\.csv$")
  clean_contents(file.path(MODEL_OUT_DIR, "model_sev"))
  clean_contents(file.path(MODEL_OUT_DIR, "model_price"))
  clean_contents(file.path(MODEL_OUT_DIR, "model_main"))
  ...
  clean_contents(APPENDIX_DIR)
  clean_contents(CTF_DIR)
  cat("  Cleaned all output directories (USE_CACHE = FALSE)\n")
}
...
if (USE_CACHE && dir.exists(CACHE_DIR)) {
  ...
  cat("  Restored", length(cache_files), "cached files (USE_CACHE = TRUE)\n")
} else {
